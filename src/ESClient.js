...
export default class ESClient {
    ...
    searchES(index, type, sort) {
        //const uri = http://localhost:9200/d3dummy/linechart/_search?q=*&sort=index:asc;
        const uri = `/${index}/${type}/_search?q=*&sort=${sort}:asc`;
    ...
    queryPostBasicSearch(index, type, postObj) {
        const uri = `/${index}/${type}/_search`;

        return this.request({method: 'POST', uri, body: postObj})
            .then(response => Request.handleResponse(response))
            .catch((error) => {
                throw new InternalServiceError('Internal Service Error', {details: (error && error.cause) || error, stack: error && error.stack});
            });
